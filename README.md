...
| The visit on the rep's Google Calendar: `gcal_events` (one row per job, the machine's memory), `gcal_sync_due()` (insert · update · move · delete · handoff) + `gcal_sync_mark()` for worker `backend/worker/gcal-sync.mjs` (`liberty-gcal.service`, every 60 s), which writes each door-born visit onto the rep's own Google Calendar through the liberty-nurture delegation, moves it on a reschedule, removes it on a cancel and steps aside when CC's own event lands on the row; ON since 18 Sep 2026 11:32 AM ET (the scope on both admin consoles, `liberty-gcal` on the box) | **404** |
| Typed into CC is CC's calendar: `gcal_sync_due` never puts a visit on Google when the job's `cc_mirror_queue` row reads by_hand or sent (CC's own event is there), and takes its own off if it already was | **405** |
| Who's free: `reps.counties` (the territory as rows; empty = anywhere), `rep_busy` + `rep_busy_reads` (every seller's Google Calendar for the next 14 days as busy blocks, replaced every five minutes by `liberty-gcal`; service role only), `gcal_busy_targets()` + `rep_busy_replace()` for the worker, `who_is_free(p_cc, p_county, p_at, p_minutes)` for the New lead door: covers · free · busy_with · visits_that_day, the same visit counted once across CC's, the door's and Google's books | **407** |
| The sub locked in: `job_sub_locks`, `sub_options`, `sub_lock` (the line on the file tags the office seat; the nugget texts the sub the contract picture and the price, RECIBIDO; a cell typed once makes the crew row), `sub_lock_mark` (typed into CC · void), `v_job_sub_locks`; the Sub locked in card on the file, the checkbox on ＋ Photo, the Office room's waiting list, the film `?tour=sub` | **397** |

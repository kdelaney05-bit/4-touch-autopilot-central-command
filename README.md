...
| The visit on the rep's Google Calendar: `gcal_events` (one row per job, the machine's memory), `gcal_sync_due()` (insert · update · move · delete · handoff) + `gcal_sync_mark()` for worker `backend/worker/gcal-sync.mjs` (`liberty-gcal.service`, every 60 s), which writes each door-born visit onto the rep's own Google Calendar through the liberty-nurture delegation, moves it on a reschedule, removes it on a cancel and steps aside when CC's own event lands on the row; inert until the Calendar scope is on the delegation | **404** |

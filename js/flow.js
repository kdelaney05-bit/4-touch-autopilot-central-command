...
async function load() {
  ...
  for (const w of ccprod) {
    ...
    for (const m of (w.material_orders || [])) if (m.delivery_at) push({ ...base, at: m.delivery_at + 'T12:00:00', pid: 'cc', body: `CC: material ${m.supplier ? 'from ' + m.supplier + ' ' : ''}delivered ${md(m.delivery_at)}${m.actual_cost ? ' · 
...
function mapHtml() {
 ...
 + Math.round(Number(m.actual_cost)).toLocaleString() : ''}`, step: 'material' });
  }

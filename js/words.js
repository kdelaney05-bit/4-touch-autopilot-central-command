...
export const MAP = [
  { key: 'signed',    seat: 'rep',         machine: ['Emails the rep, Gio, Kevin, Jess, Sam and Jonathan: SIGNED, it is moving — what opened on whom, the signed contract and the filled Notice of Commencement attached', 'Opens the paperwork checklist on the file and checks off the contract', 'Looks up the owner of record at the county', 'Emails the customer the filled Notice of Commencement to sign before a notary (the rep and the office copied) with the photo link, and texts them from the main line until the picture lands'], person: ['Nobody goes out for paperwork. The survey and the selections, if not on the file, by text or on the visit the rep already wanted'], customer: 'Signs everything on the link, once: the contract, the disclosures, the hold harmless, the permit application. Then signs the NOC in front of a notary and sends a picture from the link.' },
  { key: 'paperwork', seat: 'sold_office', machine: ['Waives the NOC and permit signature where no permit is needed (Palm Coast)', 'Opens the permit on Sam and the material order on Jonathan the second the contract signs — nothing waits on the survey, the HOA or the NOC', 'Releases the material at the signature (custom or aluminum: once the deposit is in)'], person: ['Records the customer’s notarized NOC at the Clerk when the picture lands; uploads what only she has: the HOA approval, the survey'], customer: 'Signs the NOC in front of a notary and sends the picture from the link — the machine reminds them' },

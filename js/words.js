...
const THING = {
  'CONTRACT_DOC:contract': 'the signed contract', 'CONTRACT_DOC:noc': 'the notarized Notice of Commencement', 'CONTRACT_DOC:hoa': 'the HOA approval',
...
export const MAP = [
  { key: 'signed',    seat: 'rep',         machine: ['Emails the rep, Gio, Kevin, Jess, Sam, Jonathan and Luis: SIGNED — set the visit, the notary packet attached (the NOC and the town’s forms, filled on the lines)', 'Opens the paperwork checklist on the file and checks off the contract', 'Looks up the owner of record at the county', 'Emails the customer the "you’re in" note: locked in, the order is moving, the two notary forms attached — the rep will set a time, or any notary and a picture; texts them until the forms are back'], person: ['The rep goes out once, as the notary: the NOC and the hold harmless (and the permit application where the town notarizes it), the survey and the selections if not on the file'], customer: 'Signs everything on the link, once — that is the contract. Then signs the county’s two forms in front of the rep, or at any notary with a picture back.' },
  { key: 'paperwork', seat: 'sold_office', machine: ['Fills the town\'s forms on the lines and stamps them with the signature — the hold harmless and the permit application are signed on the link, every county', 'The signed contract opens PERMIT on Sam and MATERIAL on Jonathan the same minute — nothing waits on paperwork', 'Waives the NOC and permit signature where no permit is needed (Palm Coast)', 'Reminds the rep about the NOC on days 2, 5, 9, 14, 21 and 30 until it is on the file', 'Asks the customer for a picture of the warranty deed when the county lists a different owner than the signer — email with a photo link, Sam and the rep copied, texts until it lands'], person: ['Pulls the permit off the stamped forms; records the NOC at the Clerk when the rep uploads it; uploads what only she has: the HOA approval, the survey'], customer: 'Signs everything once on the link. The NOC is the rep\'s to get notarized; the customer signs it when the rep brings it' },
  ...
  { key: 'material',  seat: 'schedule',    machine: ['Opens the material ask the moment material releases, with the calculator\'s order attached', 'Holds custom and aluminum until the card deposit is in', 'Emails the order to the supplier the products point at (switch ON, or Send to supplier on the ask), the order attached; wood waits for the permit'], person: ['Presses Send to supplier when the switch is off; enters the order number the supplier replies with'], customer: 'Nothing' },

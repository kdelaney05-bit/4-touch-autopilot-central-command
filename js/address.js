...
import { GOOGLE_MAPS_KEY } from './config.js?v=114';
import { esc } from './ui.js?v=114';
...
export function addressPicker(input, box, onPick) {
  ...
  const paint = () => {
    box.innerHTML = rows.map((r, i) => `<button type="button" class="sub${i === cur ? ' lit' : ''}" data-i="${i}"><b>${esc(r.label)}</b>${r.sub ? `<span>${esc(r.sub)}</span>` : ''}</button>`).join('');

...
const FENCE = [
  ...
  step(4, 'Pull permit', 'ask', { count: 23, lane: 'OFFICE', ask: 'PERMIT', owner: 'the office seat holding the file',
    note: 'Opens only when every paperwork piece is settled — the warranty deed included, when the county\'s owner is not who signed. Closes on the permit number. The customer gets the permit text if that switch is on.' }),
...
const ROOF = [
  ...
  step(5, 'Record NOC', 'checklist', { count: 22, lane: 'OFFICE', ask: 'CONTRACT_DOC', doc: 'noc',
    owner: 'the office seat holding the file', note: 'Part of the paperwork checklist that signing opens. It fills itself from the county record the moment the customer accepts — and if the signer is not the owner of record, the warranty deed is a checklist piece too and nothing prints until it lands.' }),

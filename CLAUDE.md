...
- **Is not:** the database. Migrations, workers and edge functions live in
  ...
  **18 Sep 2026 (afternoon):** live top is **407** — `407_whos_free` (Kevin, the minute the calendar sync was on: "how does she know if they're available?… when they want this, these are the people available" — "Who's free? That makes sense."; APPLIED LIVE by the Kevin-PC session, dry-run rolled back first; file on trureview-mobile branch `claude/whos-free`): `reps.counties` · `rep_busy` · `rep_busy_reads` · `gcal_busy_targets` · `rep_busy_replace` · `who_is_free`. This app v125: the Who's free strip on the New lead door (the address picker now carries the county beside the city, never in it). The `liberty-gcal` worker reads every seller's calendar every five minutes. **Next free = 408** — read `schema_migrations` first. ⚠️ **406** (commercial loss reasons) is live but was never written into `schema_migrations`; its guard would not stop a second 406.

  **18 Sep 2026 (12:55 PM):** this app **v126** and a second **407** on live — `407_ask_hand` (this session; file on trureview-mobile main; the who's-free session's `407_whos_free` is on its PR branch — two 407s, like the two 369s and 381s). Jess's "Processing new sold jobs" email: every document on the file opens (Open on each On-the-file row and each settled Paperwork row; `docUrl` in `book.js`: `estimates/…` is a signed link, `<company>/<project>/…` is job-docs, public); **Hand to…** on every open ask (the file's Asks card, the Office room's queue) over `ask_hand(p_ask, p_to, p_note)`. The seats door (`stage_seats`) still decides who gets every NEW ask. **v127 (1:00 PM):** `job-docs` is a PRIVATE bucket (`uploadPublic` is a misnomer) — `docUrl` signs it (`storage.objects` policy "job docs read" = `can_see_lane` on the path's project); the thread's document line opens; the settled line uses `closed_by`. Rows the same minute (Kevin: "HOA should never default"): `brand_text_settings.checklist` = contract,noc,survey,permit_sig for 1461 and contract,noc,permit_sig for 1537/1563. **v130 (1:20 PM):** `enterPosts` — Enter is a new line, Ctrl+Enter or the button sends (Jess: half-typed notes went out); the settle dialog's file input is `required` only when the rule is not waivable (the browser refused the waive); a VOID Paperwork row reads "off the list"; the `thread_asks` select carries `void_reason, closed_by`. **v131 (1:25 PM):** `newerVersion()` in `app.js` reads `index.html` every three minutes and on tab focus; a higher `app.js?v=` shows `#btn-newer` (Jonathan's stale page, 1:01 PM). **v132 (1:45 PM):** the @ picker (`wireAtOn` in `village.js`, shared by the Village, Say it and the file's note box) lists employees first with `titleOf(p)` (from `reps.team`, `manages_company_id`, the production watcher in `stage_seats`) and a tinted row, customers after with a CUSTOMER tag; `enterPosts` reads `localStorage.enter_sends` (the switch `#enter-sends` in the who row). **v133 (2:00 PM):** the Village's `read()` merges the bells from `hype_messages` (`body like *just hung one*`, `BELLS = 12`) as gold rows; `cheer()` and a bell reply insert into `hype_messages` with `reply_to` (the reps' thread, `hype_push_fanout` carries it). Kevin's idea; no migration. **v134 (2:15 PM) + migration 409 `409_my_character`** (APPLIED LIVE by this session: `reps.color` + `reps_color_hex`; file on trureview-mobile main): `personOf` · `hueOf` · `charStyle` · `charFace` in `village.js` paint every bubble and @ row from `reps.avatar`/`reps.color` or a hash of the name; `#my-character` in the Village tab PATCHes the seat's own row (`rep_updates_self`); the Village header is one line and the room is `calc(100vh - 290px)`. **v135 (3:40 PM) + migration 410 `410_the_office_sees_every_text`** (APPLIED LIVE by this session; file on trureview-mobile main): RLS `text_messages_office` + `sms_outbox_office_reads` (Jess 2:52 PM: Laura and Sam could not see the texts on a file); `ask_void(p_ask, p_reason)` behind **Not needed** on every open ask (the file's Asks card, the Office room); the Office room's `laneF` (Office · Field · Everything) and `oldOnly` (3 days +); `senderOf(t, job, o)` in `file.js` names the seat from the matching `sms_outbox` row; trigger `zz_ask_email_the_holder` mails a new ask to a holder with no `rep_push_tokens` via `rep_email_queue` (kind `ask_opened`, `office: true`, never CONTRACT_DOC) from the opener's mailbox — CC's task email, kept (Jess 3:13 PM). **Migration 411 `411_the_signed_link_posts_itself`** (APPLIED LIVE 18 Sep ~4:00 PM by this session; file on trureview-mobile main; no app change): trigger `zz_signed_posts_to_the_village` on `customer_signatures` — a Village post as the rep who sent the link (`team_messages`, `customer_id` set, the amount from `estimates` by the link's token) and `jobs.contract_signed_at`/`fin_sold_amount` stamped when null; practice names and `reporting_excluded` never post. The first real signature on the link was 18 Sep 3:27 PM (Travis). **v141 (4:35 PM) + migration 413 `413_sunbiz_before_the_deed`** (APPLIED LIVE by this session ~4:25 PM as 412, renamed 413 in `schema_migrations` when the Kevin-PC session's `412_the_contract_on_the_file` landed first; file on trureview-mobile main): `deed_request_start` gains `p_force` — an entity owner (`signer_match = 'entity'`) opens a Sunbiz ask and sends no email unless forced (`deed_request_send` forces; Sam: "I looked at Sunbiz… can we not do this one"); `paperwork_filled_for` carries `storage_path`; the deed switch's words in `office.js`. Two 411s and two 412s are live (like the two 369s, 381s and 407s). **Migration 414 `414_nobody_on_it_till_they_need_to_be`** (APPLIED LIVE 18 Sep ~5:05 PM by this session; no app change): `signed_set_the_visit` emails the rep + the office seat only, `deed_request_email` the customer + rep + office seat, Kevin off `machine_email_watchers`; 410b the same hour: `ask_email_the_holder` skips a self-opened ask and falls back to the owner's mailbox, never the office seat's. **Next free = 415.** **Next free = 408** — read `schema_migrations` first.

  **18 Sep 2026 (4:15 PM):** live top is **412** — `412_the_contract_on_the_file` (Kevin, 3:45 PM: "our very first digital signed contract today… the contract didn't show up. We can't see it." — Joseph Auxilly, Palm Bay, Travis's photo estimate; APPLIED LIVE by the Kevin-PC session, dry-run rolled back first; file on trureview-mobile main): the office reads the private `estimates` bucket (`office_reads_estimates`), `paperwork_filled_for` for the office, a stamped 'contract' becomes the contract ask's proof and the thread's one contract. paperwork-fill builds a photo/PDF estimate's contract on the server (the picture + the ACCEPTED AND SIGNED page; `rebuild_contract: true` re-makes one); estimate-view's `/pdf` carries CORS and `/contract` is skipped for a non-doc link. This app **v140**: FORM_LABEL knows the contract, stamped rows read "signed on the link", the Filled-from-the-file Open goes through paperwork-fill's `open`. Nobody re-signs. **Next free = 413** — read `schema_migrations` first.
  **18 Sep 2026 (5:05 PM):** this app **v142** — THE BING ON EVERY MESSAGE (Kevin, 4:50 PM: "I want everyone at the company to be dinging when they're being notified. That's their notification, not a call."): `alerts.js` now polls `v_team_room` and `hype_messages` too (posts by others since the page opened, the machine's silent seats skipped, the pane on the screen skipped) and fires the same chime + card + browser notification; `#bing-all` "Every message" in the rail (localStorage `cc-bing-all`, default ON) turns it down to tags only; `openKey()` in `threads.js` and `mountedRooms()` in `village.js` say what is on the screen; `?demo=1&bing=all` stages two cards for the picture. No migration. Found the same afternoon, NOT fixed: (1) the file thread paints a TEXT reply as "✉ EMAIL REPLY" (the engine sweep's `reply` event has no channel; Haakon / Erin Gansert, 4:25 PM); (2) `nurture_merge_ctx` greets a "Last, First" customer by the last name (`split_part(name, ' ', 1)` instead of `nurture_first_name`): 20 machine emails to 12 customers since 17 Sep. Both are one small migration each; live top is **413** (`413_sunbiz_before_the_deed`, a sibling) — **next free = 414**.

...
- **Is not:** the database. Migrations, workers and edge functions live in
  ...
  **18 Sep 2026 (12:55 PM):** this app **v126** and a second **407** on live — `407_ask_hand` (this session; file on trureview-mobile main; the who's-free session's `407_whos_free` is on its PR branch — two 407s, like the two 369s and 381s). Jess's "Processing new sold jobs" email: every document on the file opens (Open on each On-the-file row and each settled Paperwork row; `docUrl` in `book.js`: `estimates/…` is a signed link, `<company>/<project>/…` is job-docs, public); **Hand to…** on every open ask (the file's Asks card, the Office room's queue) over `ask_hand(p_ask, p_to, p_note)`. The seats door (`stage_seats`) still decides who gets every NEW ask. **Next free = 408** — read `schema_migrations` first.

...
| A charge on the company card (Kevin, 16 Sep 10:30 PM: "do you need my credit card statements? we can automate that too. I wait for Marianna forever to check boxes") | the card's activity export (Chase CSV, monthly or on demand); each charge matched to a payee (Home Depot, Lowes, the permit counter, fuel) and to the seat's job that day | **CARD CHARGE LANDED** · who swiped · which job, or overhead | the card holder (Sam, Jonathan, Kevin) | next lane: Approve writes the QuickBooks Purchase with the customer:job, and QuickBooks matches it to the bank feed by itself, so Marianna's boxes shrink to the leftovers. The QuickBooks API cannot read or tick the bank-feed review queue; entering the expense with the job is what makes the match. Needs: one 90-day CSV per card and who holds each card. |
...
1. ~~Whether a supervisor may land a crew's invoice~~ **Decided 16 Sep 10:50 PM: the office (Jess) lands the crew's paper first through the "+ Bill" door; the supervisors' snap in their app comes a week later.** The door allows office, manager, owner, admin either way.

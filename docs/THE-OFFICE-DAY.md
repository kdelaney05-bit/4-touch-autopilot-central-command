...
| 3 | Upload Contract and create sales order | **Input 1: upload the paperwork.** Contract, NOC, permit signature, HOA, survey. Each item closes on its upload. Not needed on this job? Waive it with a reason. | office |
...
| 5 | Locate scheduled | **Input 3: the locate date.** Opened by itself when the permit landed. | office |
| 6 | Order Materials and Schedule Labor | **Input 4: the start date** (and the material order). Both opened by themselves when the locate closed. The date sends the customer the schedule text and pushes production. | Jonathan |

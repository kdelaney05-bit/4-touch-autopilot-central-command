...
## READ THIS FIRST — corrections from the new account, 16 Sep ~6 PM

Checked against the live feed (`uvoice_sms_raw`, `text_messages`), Kevin's Gmail and Uvoice's latest ticket entries. Four things below are wrong or stale.

1. **Pro-Tech campaign is COMPLETE, not "resubmitting".** Dwayne's 10:34 AM "in progress" was superseded by Oliver on ticket T20260910.0005 at 12:08 PM: campaign registered, 321-352-6955 (main), 321-783-1688 (Jermey) and 321-783-1694 (office, shared to Jess/Sam/Laura) all assigned and "copying to the URL". Two new CloudMessage accounts exist for Pro-Tech; their logins are on that ticket (not copied here — no secrets in the repo). API UIDs/tokens are pending; Kevin's account replied at 4:24 PM: send them on the ticket. As of 6 PM nothing has ever posted to our URL from 321-352-6955 or 321-783-1688 (untested, not broken). 321-783-1694 posts from the PBX log forward only (12 posts, newest 5:49 PM) — expected for a ConnectUC-shared line.
2. **321-806-1995: texts DO reach our URL — from the PBX log forward only, never from CloudMessage.** Uvoice posts every text twice: the PBX copy (`source`/`destination`/`destinationuser`) and the CloudMessage copy (`MessageID`/`ConversationID`/`ToNumber`). Per line, ever: 386-276-6898 = 839 PBX + 499 CloudMessage; Oasis 321-274-4268 = 2 + 2; Mike 321-220-9556 = 10 + 5; **321-806-1995 = 13 PBX + 0 CloudMessage.** Kevin's three tests (9/15 12:24 PM, 9/15 5:23 PM, 9/16 11:06 AM ET) all arrived via the PBX copy and were ingested into `text_messages` (feed `uvoice`) with **no `to_number` and no extension**, so nothing can put them on the 321 line. So "setup is correct" is true of the PBX and false of CloudMessage. The ask to Uvoice changed from "send a test" to "confirm 321-806-1995 is in the Liberty CloudMessage account, on the campaign, with the URL forward on — then one test." Reps stay on 386 until a CloudMessage post arrives for it.
3. **Oasis 321-274-4268 is already proven.** Kevin's tests on 9/14 5:58 PM and 9/16 11:05 AM arrived through both copies and landed in `text_messages` as `cloudmessage` inbound. No further test needed. App side: `sms_lines.active` = false and `brand_sms_lines.campaign_ok` = false for Oasis; flipping is Kevin's call (Dwayne confirmed the *brand* 9/2; confirm the *campaign* before reps text from it — the 311 law).
4. **There is no "Re: Responses and suggestions." draft.** The Dwayne draft that exists was written 9/15 10:55 PM, before Dwayne answered, and said "nothing reached our URL" and "Dwayne is resubmitting" — both now wrong. Rewritten 16 Sep evening as a reply to Dwayne's 10:34 AM email with the facts above; the T20260914.0005 keep-open draft was rewritten the same way. Both still unsent. The other six drafts in the list are untouched.

Also newer than this handoff: the second auto-close warning on T20260914.0005 (4:36 PM) — the keep-open draft answers it. Four close-out emails hit Kevin's inbox 5:18–5:28 PM (this one, the app handoff, the pickup list, Jake's Terminator); read all four.

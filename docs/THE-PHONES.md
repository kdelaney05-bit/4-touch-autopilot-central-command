...
## 18 Sep 2026, 12:00–12:50 PM ET: the decisions on the phone with Uvoice, and what shipped (Kevin: "I want it all to go to iPhone… it's the outbounds that matter")

**Uvoice's side, done by Jeff at 12:10 PM:** every rep's line forwards to the rep's iPhone when ConnectUC is not registered (Kevin's list: 150 Eric, 151 Travis, 152 Ron, 154 Haakon, 156 Mike; the iPhone numbers are on each `rep_channel_map` note, not here). Which rule Jeff set is not yet confirmed: "forward when offline" catches the straight-to-voicemail shape only; "forward when unanswered" catches the rang-out shape too. **Ask Dwayne which.** A forwarded call on the cell is a cellular call: no Wi-Fi hop can drop it mid-call, and there is no gap a caller hears, because the PBX decides where to ring before it rings.

**Our side, 12:20 PM (rows, not code):** `reps.uvoice_live_at` cleared on the seven ConnectUC seats (Travis, Eric, Haakon, Ron, Mike, Gustavo, Jermey; they were 1 Sep 11:30 ×3, 1 Sep 14:50, 16 Sep 18:42, 17 Sep 10:49, 18 Sep 08:14). The app's Call button now opens the iPhone's own dialer (App.tsx `handoffCall`, the pre-port path). Texting did not move: every one of the seven has `sms_via = cloudmessage`, so Text stays on the brand line inside the app whatever the flag says. Kevin's ruling, said plainly: outbound calls from the iPhone leave Uvoice's file; customers will learn the iPhone number from caller ID and may text it (that lands in iMessage, not on the file); he chose it. Mike's 17 Sep "leave the poor man alone" was about his texting line, which is untouched; his Call button now dials from 321-317-8383.

**408 `the_call_from_the_iphone`** (trureview-mobile main e467c7a; APPLIED LIVE 12:40 PM through the Supabase migration tool, dry-run rolled back first): `app_calls` (rep · customer matched by the number when the app did not say · direction out/in · via iphone/manual · outcome talked/voicemail/cancelled), `app_call_log()`, `app_call_outcome()`, RLS like `uvoice_calls`. **The app (OTA production, 12:37 PM):** on Call, the tap is logged before the dialer opens; when the app comes back to the front it asks "Your call to (321) …: Voicemail · Talked to them" (back inside 6 s means the rep cancelled iOS's dial sheet: marked cancelled, no question); **Log a call** on every customer card for a call made outside the app (which way, then the same question); the chip never counts as a touch. **Central Command v126:** `js/book.js` loads `app_calls` for the file and `js/file.js` draws "📞 Travis called them · talked to them · from the iPhone" / "voicemail" / "not logged yet" / "logged by hand"; cancelled dials are not drawn. **Found on the way:** since 401 the file loaded `uvoice_calls` and never returned them, so live files never drew a Uvoice call; fixed in the same line (the demo had them, which is why nobody saw it).

**What the hourly file shows from now on:** inbound legs still land under the rep's extension (forwarded calls should appear; read the 1 PM and 2 PM files and fix the typing in `uvoice-cdr.mjs` if a forwarded leg reads wrong); outbound from the iPhone never appears there — `app_calls` is the outbound record. The missed-call buzz (401) still fires for missed rows.
